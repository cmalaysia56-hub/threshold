...
export default function Threshold() {
  ...
  // Firebase auth state is only known on the client, and it's often already
  // resolved (from a cached session) by the time React hydrates — while the
  // server always renders as signed-out. That mismatch between server HTML
  // and the client's first paint is what throws React hydration errors
  // #418/#423/#425, which then stop the whole page from responding to any
  // clicks (including "Save to your journal"). `mounted` keeps the very
  // first client render identical to the server's, then reveals the real
  // auth state right after, avoiding the mismatch entirely.
